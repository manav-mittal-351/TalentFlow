...
// Reusable job editor form layout for both Job Creation and Job Modification views.
// Integrates React Hook Form, Zod client validations, and accessibility standards.
...
export function JobForm({ defaultValues, onSubmit, isPending, submitButtonText = 'Save Role' }) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  // Sync defaultValues if they are fetched asynchronously (e.g. on Edit Job load)
  useEffect(() => {
    if (defaultValues) {
      reset(defaultValues);
    }
